...
import com.example.Lodgify.MethodFormer;
...
public class Booking implements Tools, Regex, MethodFormer {
    ...
    public void updateCustomerDetails() {
        ...
        updateOptions.setOnAction(event -> {
            ...
            if (selectedOption != null) {
                ...
                showError( "Please select an update option.");
            }
        });
                ...
                if (rBook.get(i).getRNumber() == roomN) {
                    ...
                    okButton.addEventFilter(ActionEvent.ACTION, e -> {
                        ...
                        if (!name.matches(NAME)) {
                            showError( "Name must contain only alphabetic characters and spaces.");
                            e.consume();
                            return;
                        }

                        if (!phone.matches(PHONE)) {
                            showError( "Phone Number must be in the format XXXX-XXXXXXX.");
                            e.consume();
                            return;
                        }
                        if (!email.matches(GMAIL)) {
                            showError( "Email must be in the format username@example.com.");
                            e.consume();
                            return;
                        }
                        if (!dob.matches(DOB)) {
                            showError( "Date of Birth must be in the format YYYY-MM-DD.");
                            e.consume();
                            return;
                        }

                        //duplication
                        for(PersonWhoBook p1: pBook){
                            if(p1 != person){
                                if(p1.getPhoneNumber().equals(phone)) {
                                    e.consume();
                                    showError( "Customer with this Phone Number already exists.");
                                    return;

                                }
                                if(p1.getGmail().equals(email)){
                                    showError("Customer with this Gmail already exists.");
                                    e.consume();
                                    return;
...
//                    pBook.set(i, person); // Update the ArrayList (testing phase)
            ...
            if (!found) {
                showError("No data found for the given Room Number.");
            }
        });
    }
    ...
    private void updateByCnic() {
        ...
        inputDialog.showAndWait().ifPresent(event -> {
            ...
            if (!cnic.matches(CNIC)) {
                showError("CNIC must be in the format XXXXX-XXXXXXX-X.");
                return;
            }
            ...
            for (int i = 0; i < pBook.size(); i++) {
                ...
                if (cnic.equals(pBook.get(i).getCnic())) {
                    ...
                    okButton.addEventFilter(ActionEvent.ACTION, e -> {
                        ...
                        for(PersonWhoBook p1: pBook){
                            if(p1 != person){
                                if(p1.getPhoneNumber().equals(phoneNumber)) {
                                    e.consume();
                                    showError("Customer with this Phone Number already exists.");
                                    return;
...
//                    pBook.set(i, person); testing purposes
            ...
            if (!found) {
                showError("No data found for the given CNIC.");
            }
        });
    }
    ...
    public void updateHygienicStatus() {
        ...
        updateButtonNode.addEventFilter(ActionEvent.ACTION, event -> {
            ...
            try {
                ...
                showError( "Please enter a valid integer for room number.");
            ...
            for (Room room : Booking.rLs1) {
                if (room.getRNumber() == number) {
                    ...
                    if (newStatus.isEmpty()) {
                        showError("Please enter a new hygienic status.");
                        event.consume(); // Prevent dialog from closing
                        return;
                    }
            ...
            if (!found) {
                showError( "No room found with the given number.");
                event.consume(); // Prevent dialog from closing
            }
        });
